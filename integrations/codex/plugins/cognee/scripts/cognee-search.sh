...
#
# Configuration:
#   Reads resolved session ID and dataset from ~/.cognee-plugin/codex/resolved.json
#   (written by SessionStart hook). Falls back to env vars.

set -euo pipefail

RESOLVED_FILE="${HOME}/.cognee-plugin/codex/resolved.json"

# Read resolved config if available, otherwise use env var defaults
if [ -f "$RESOLVED_FILE" ]; then
    DATASET=$(python3 -c "import json; print(json.load(open('$RESOLVED_FILE')).get('dataset','codex_sessions'))" 2>/dev/null || echo "codex_sessions")
    SESSION_ID=$(python3 -c "import json; print(json.load(open('$RESOLVED_FILE')).get('session_id','codex_session'))" 2>/dev/null || echo "codex_session")
else
    DATASET="${COGNEE_PLUGIN_DATASET:-codex_sessions}"
    SESSION_ID="${COGNEE_SESSION_ID:-codex_session}"
fi
