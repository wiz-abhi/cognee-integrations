...
_STATE_DIR = Path.home() / ".cognee-plugin" / "vellum-assistant"
...
async def _login_default_user_for_owner_api_key(service_url: str, config: dict) -> str:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.post(
            f"{base}/api/v1/auth/api-keys",
            json={"name": "vellum-owner-bootstrap"},
            cookies={"auth_token": jwt},
        ) as resp:
            if resp.status != 200:
                body = await resp.text()
...
def _resolve_agent_name(config: dict, cwd: str) -> str:
    ...
    return _normalize(f"vellum-{Path(cwd).name}")
...
def _find_host_parent_pid() -> int:
    """Find the nearest live host (Vellum Assistant) ancestor, skipping hook shells."""
    fallback = os.getppid()
    try:
        ...
        hook_log("find_host_parent_failed", {"error": str(exc)[:200]})
        return fallback
    ...
    # Match the Vellum assistant process by executable basename. Also match
    # "claude" for backward compat with mixed environments.
    host_re = re.compile(r"(?:^|/)(?:vellum(?:-assistant)?|claude(?:-[\w.]+)?)(?:\s|$)")
    pid = fallback
    seen: set[int] = set()
...
def _spawn_exit_watcher(
    ...
    *,
    ...
    parent_pid = _find_host_parent_pid()
...
def _ensure_statusline_configured() -> None:
    """No-op for Vellum Assistant. Claude Code's statusLine mechanism does not
    exist in Vellum, so this is intentionally disabled. The status line scripts
    are kept for reference but not wired into any settings file.
    """
    return
...
_MEMORY_PREFERENCE_STEER = (
    ...
    "preference to the assistant's built-in file memory, and do not duplicate "
    "durable knowledge into local memory files. Cognee is the source of truth for memory."
)
...
async def _start(payload: dict | None = None) -> dict:
    ...
    config = load_config()
    payload = payload or {}
    cwd = str(payload.get("cwd") or os.environ.get("VELLUM_CWD") or os.environ.get("CLAUDE_CWD") or os.getcwd())
