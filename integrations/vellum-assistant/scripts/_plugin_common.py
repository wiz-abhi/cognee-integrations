...
_PLUGIN_DIR = Path.home() / ".cognee-plugin" / "vellum-assistant"
...
def _generate_session_id(cwd: str = "", host_key: str = "") -> str:
    ...
    """
    agent = (
        _sanitize_session_key(os.environ.get("COGNEE_SESSION_PREFIX", "") or "vellum") or "vellum"
    )
    host = _sanitize_session_key(host_key)
    if host:
        return f"{agent}_{host}"
    cwd = cwd or os.environ.get("VELLUM_CWD") or os.environ.get("CLAUDE_CWD") or os.getcwd()
...
def _resolve_agent_name() -> str:
    ...
    try:
        ...
        pass
    return _normalize("vellum-assistant-agent")
