"""Tests for the 'prefer Cognee memory' init steer (session-start.py).

The assistant's native file memory can't be reliably disabled by a plugin, so
session-start injects an ``additionalContext`` instruction asserting Cognee as
the preferred memory. These drive ``_apply_memory_preference`` directly. The
session-start module pulls in hook helpers; if it can't import in this
environment the tests skip (return) rather than fail.

Run: python integrations/vellum-assistant/tests/test_memory_preference.py
(or via pytest).
"""
